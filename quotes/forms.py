...
class QuoteForm(forms.ModelForm):
    ...
    new_source = forms.CharField(
        max_length=100, required=False, label="Новый источник (название)"
    )
    new_source_type = forms.ChoiceField(
        choices=Source.SOURCE_TYPES, required=False, label="Тип источника"
    )
    ...
    def clean(self):
        ...
        if source:
            ...
            final_source, created = Source.objects.get_or_create(
                name=new_source_name.strip(), defaults={"type": source_type}
            )
        if final_source is None:
            raise forms.ValidationError(
                "Произошла ошибка при определении источника. Попробуйте еще раз."
            )
